...
async function promptForSettings(flags, mode) {
  const settings = {}
  const cfg = core.readConfig()
  if (!flags.path && !flags.out) {
    const defaultPath = cfg?.downloadPath || core.determinePath() || path.join(__dirname, 'downloads')
    const { pathChoice } = await inquirer.prompt([{
      type: 'list',
      name: 'pathChoice',
      message: 'Where should this be saved?',
      choices: [
        { name: `Default: ${defaultPath}`, value: 'default' },
    ...
    if (pathChoice === 'custom') {
      ...
      settings.path = customPath.trim()
    } else {
      settings.path = defaultPath
    }
  }

  if (!flags.folder && !flags.f && mode !== 'movie') {
    const { folder } = await inquirer.prompt([{
      type: 'confirm',
      name: 'folder',
      message: 'Organize into folders?',
      default: false
    }])
    settings.folder = folder
  }

  if (!flags.s && mode !== 'anime') {
    const { subtitles } = await inquirer.prompt([{
      type: 'confirm',
      name: 'subtitles',
      message: 'Download and embed subtitles? (saved to /subtitles/)',
      default: true
    }])
    settings.s = subtitles
  }

  if (!flags.aac && !cfg?.autoTranscode) {
    const { aac } = await inquirer.prompt([{
      type: 'confirm',
      name: 'aac',
      message: 'Convert audio to AAC?',
      default: false
    }])
    settings.aac = aac
  }

  if (!flags.autoplay && !isTermux() && mode !== 'movie') {
    const { autoplay } = await inquirer.prompt([{
      type: 'confirm',
      name: 'autoplay',
      message: 'Autoplay after download?',
      default: false
    }])
...
async function animeFlow(flags) {
  ...
  const settings = await promptForSettings(flags, 'anime')
  Object.assign(flags, settings)
  ...
  if (flags.wholeshow) {
    ...
    for (const { ep, idx } of episodeList) {
      ...
      const recode = flags.aac || (flags.transcode || flags.recode) ? true : (cfgCore && cfgCore.autoTranscode);
      ...
      if (subCount > 0) console.log(`Subtitles: ${subCount} track(s) embedded (source files in /subtitles/)`);
  ...
  if (!flags.ep || targets.length === 0) {
    ...
    const topChoices = [
      { name: 'Download All Episodes', value: 'all' },
      { name: 'Download Range', value: 'range' },
      new inquirer.Separator()
    ]
    const { pick } = await inquirer.prompt([{ type: 'list', name: 'pick', message: 'Select episode', choices: [...topChoices, ...epChoices], pageSize: Math.min(epChoices.length + 3, cfg.pageSizeDefault || 20) }])
    if (pick === 'all') {
      targets = episodes
      flags.wholeshow = true
    } else if (pick === 'range') {
      ...
      if (pr.includes('-')) {
        ...
        if (targets.length > 1) flags.wholeshow = true
      } else {
  ...
  if (flags.wholeshow && targets.length > 1) {
    console.log('Queueing batch download for', targets.length, 'episodes')
    const progressStates = {};
    const episodeList = targets.map((ep, idx) => ({ ep, idx }));
    let allOpts = [];
    for (const { ep } of episodeList) {
      const playUrl = `https://animepahe.si/play/${encodeURIComponent(slug)}/${ep.session}`;
      try {
...
async function seriesFlow(flags) {
  ...
  const settings = await promptForSettings(flags, 'series')
  Object.assign(flags, settings)
    ...
    for (const seasonObj of targetSeasons) {
      ...
      for (const ep of targetEpisodes) {
        ...
        try {
          ...
          const recode = flags.aac || (flags.transcode || flags.recode) ? true : (cfg && cfg.autoTranscode);
          fileName = normalizeEpisodeFileName(String(fileName || ''))
          const _base = sanitizeForFile(fileName)
          ...
          console.log(`✅ Saved: ${outPath}`);
          if (subCount > 0) console.log(`💬 Subtitles: ${subCount} track(s) embedded (source files in /subtitles/)`);
          if (flags.autoplay && !isTermux()) {
            try {
              const open = require('open');
              await open(outPath, { wait: false });
            } catch (e) {}
          } else if (flags.autoplay && isTermux()) {
            console.log('Autoplay is not supported in Termux. Please open the file manually with mpv or your preferred player.');
          }
  ...
  const { epSelect } = await inquirer.prompt([{ type: 'list', name: 'epSelect', message: 'Select episode', choices: [...topChoices, ...epChoices], pageSize: Math.min(epChoices.length+3, cfg.pageSizeDefault || 20) }])
  let targets = []
  if (epSelect === 'all') {
    targets = eps
  } else if (epSelect === 'range') {
    const { r } = await inquirer.prompt([{ name: 'r', message: 'Range (e.g. 1-5):' }])
    const pr = (r||'').trim()
    if (pr.includes('-')) {
      const [s,e] = pr.split('-',2).map(x=>parseInt(x,10))
  ...
  for (const t of targets.filter(Boolean)) {
    try {
      ...
      const recode = flags.aac || (flags.transcode || flags.recode) ? true : (cfg && cfg.autoTranscode);
      let outPath = path.join(dest, fileName + '.mp4');
      try { await downloader.download(dest, fileName, decoded, refer, { recodeAudio: !!recode, subtitles: (flags.s && subs && subs.length) ? subs : undefined }); } finally { unregisterDownload(_base) }
      if (flags.autoplay && !isTermux()) {
        try {
          const open = require('open');
          await open(outPath, { wait: false });
...
async function movieFlow(flags) {
  ...
  if (!query) return
    ...
    if (!sel) return
      ...
      const settings = await promptForSettings(flags, 'movie')
    Object.assign(flags, settings)
...
async function main() {
  ...
  const hasModeFlag = flags.anime || flags.tv || flags.series || flags.mtv || flags.movie || flags.m
